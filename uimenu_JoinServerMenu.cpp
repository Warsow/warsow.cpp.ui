...
JoinServerMenu::JoinServerMenu() : MainMenu( "Join A Game" )
{
	...
	for ( Iter it = begin ; it != end ; ++it )
	{
		...
		if( checkFilters( ptr ) )
		{
			...
			else
			{
				...
				l = Factory::newLabel( item, 0, 0, PING_COL_WIDTH, item->getHeight(), va("%d", ptr->ping) );
				l->setAlign( ALIGN_MIDDLE_CENTER );
				l->setDisabledFontColor( l->getFontColor() );
				l->setEnabled( true );
				l->setClickThrough( true );
				if (ptr->bots) {
					l = Factory::newLabel( item, xoffset += PING_COL_WIDTH, 0, PLAYER_COL_WIDTH, item->getHeight(), va("%d/%d (%d bot%s)", (ptr->curuser - ptr->bots), ptr->maxuser, ptr->bots, ((ptr->bots == 1) ? "" : "s") ) );
				} else {
					l = Factory::newLabel( item, xoffset += PING_COL_WIDTH, 0, PLAYER_COL_WIDTH, item->getHeight(), va("%d/%d", ptr->curuser, ptr->maxuser) );
				}
				...
				if ( ptr->instagib )
					l = Factory::newLabel( item, xoffset += PLAYER_COL_WIDTH, 0, GAMETYPE_COL_WIDTH, item->getHeight(), va("i%s", ptr->gametype.c_str()) );
				else
					l = Factory::newLabel( item, xoffset += PLAYER_COL_WIDTH, 0, GAMETYPE_COL_WIDTH, item->getHeight(), ptr->gametype );
				l->setAlign( ALIGN_MIDDLE_CENTER );
				l->setDisabledFontColor( l->getFontColor() );
				l->setEnabled( true );
				l->setClickThrough( true );

				l = Factory::newLabel( item, xoffset += GAMETYPE_COL_WIDTH, 0, MAP_COL_WIDTH, item->getHeight(), ptr->map );
				l->setAlign( ALIGN_MIDDLE_CENTER );
				l->setDisabledFontColor( l->getFontColor() );
				l->setEnabled( true );
				l->setClickThrough( true );

				l = Factory::newLabel( item, xoffset += MAP_COL_WIDTH, 0, NAME_COL_WIDTH, item->getHeight(), ptr->hostname );
				l->setAlign( ALIGN_MIDDLE_LEFT );
				l->setDisabledFontColor( l->getFontColor() );
				l->setEnabled( true );
				l->setClickThrough( true );
...
void JoinServerMenu::AddToServerList( const char *adr, const char *info )
{

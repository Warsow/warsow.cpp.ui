...
bool PlayerModels::isValidModel( char *model_name )
{
	...
	Q_snprintfz( scratch, sizeof(scratch), "models/players/%s/tris.iqm", model_name );
	found = (bool)( Trap::FS_FOpenFile(scratch, NULL, FS_READ) != -1 );
	if( !found )
		return false;
...
void PlayerModels::CreatePlayerModelList( void )
{
	...
	ndirs = Trap::FS_GetFileList( "models/players", "/", NULL, 0, 0, 0 );
	if( !ndirs )
		return;
	...
	i = 0;
	do {
		if( (k = Trap::FS_GetFileList( "models/players", "/", dirnames, sizeof( dirnames ), i, ndirs )) == 0 )
		{
			i++;
			continue;
		}
...
void PlayerModels::DrawPlayerModel( const char *model, const char *skin, byte_vec4_t color, int xpos, int ypos, int width, int height, int frame, int oldframe )
{
	...
	Q_snprintfz( scratch, sizeof( scratch ), "models/players/%s/tris.iqm", model );

...
SetupGraphicsMenu::SetupGraphicsMenu()
{
	...
	fullscreen = Factory::newCheckBox( basic, 155, yoffset[0], "Fullscreen", 125 );
	vsync = Factory::newCheckBox( basic, 155, yoffset[0] += 35, "Vertical sync", 140 );
	...
	*/	
	...
	perPixelLighting = Factory::newCheckBox( basic, 20, yoffset[0] += 30, "Per pixel lighting", 150 );
	...
	simpleItems = Factory::newCheckBox( advanced, 20, yoffset[1] += 25, "Simple Items", 155 );
	simpleItems->setAlign( ALIGN_MIDDLE_LEFT );

	wallMarks = Factory::newCheckBox( advanced, 20, yoffset[1] += 25, "Marks on walls", 155 );
	wallMarks->setAlign( ALIGN_MIDDLE_LEFT );

	bloom = Factory::newCheckBox( advanced, 20, yoffset[1] += 25, "Light bloom", 155 );
	bloom->setAlign( ALIGN_MIDDLE_LEFT );

	selfShadow = Factory::newCheckBox( advanced, 20, yoffset[1] += 25, "Show self shadow", 155 );
	selfShadow->setAlign( ALIGN_MIDDLE_LEFT );

	reliefMapping = Factory::newCheckBox( advanced, 20, yoffset[1] += 25, "Relief mapping", 155 );
	reliefMapping->setAlign( ALIGN_MIDDLE_LEFT );

	portalMaps = Factory::newCheckBox( advanced, 20, yoffset[1] += 25, "Portalmaps", 155 );
	portalMaps->setAlign( ALIGN_MIDDLE_LEFT );

	specularLighting = Factory::newCheckBox( advanced, 20, yoffset[1] += 25, "Specular lighting", 155 );
	specularLighting->setAlign( ALIGN_MIDDLE_LEFT );

	fastSky = Factory::newCheckBox( advanced, 20, yoffset[1] += 25, "Fast sky", 155 );

...
namespace UIWsw
{
	...
	public:
		...
		static inline void R_AddLightToScene( vec3_t org, float intensity, float r, float g, float b ) {
		...
		static inline void R_RegisterWorldModel( const char *name ) {
		...
		static inline struct shader_s *R_RegisterLevelshot( const char *name, struct shader_s *defaultPic, bool *matchesDefault ) {
		...
		static inline bool R_LerpTag( orientation_t *orient, struct model_s *mod, int oldframe, int frame, float lerpfrac, const char *name ) {
		...
		static inline void R_DrawRotatedStretchPic( int x, int y, int w, int h, float s1, float t1, float s2, float t2, float angle, vec4_t color, struct shader_s *shader ) {
...
//		}
		...
		static inline void S_StartBackgroundTrack( const char *intro, const char *loop ) {
		...
		static inline struct qfontface_s *SCR_RegisterFont( const char *name, int style, unsigned int size ) {
			return UI_IMPORT.SCR_RegisterFont( name, style, size );
		}

		static inline void SCR_DrawString( int x, int y, int align, const char *str, struct qfontface_s *font, vec4_t color ) {
			UI_IMPORT.SCR_DrawString( x, y, align, str, font, color );
		}

		static inline int SCR_DrawStringWidth( int x, int y, int align, const char *str, int maxwidth, struct qfontface_s *font, vec4_t color ) {
			return UI_IMPORT.SCR_DrawStringWidth( x, y, align, str, maxwidth, font, color );
		}

		static inline void SCR_DrawClampString( int x, int y, const char *str, int xmin, int ymin, int xmax, int ymax, struct qfontface_s *font, vec4_t color ) {
			UI_IMPORT.SCR_DrawClampString( x, y, str, xmin, ymin, xmax, ymax, font, color );
		}

		static inline size_t SCR_strHeight( struct qfontface_s *font ) {
			return UI_IMPORT.SCR_FontHeight( font );
		}

		static inline size_t SCR_strWidth( const char *str, struct qfontface_s *font, int maxlen ) {
			return UI_IMPORT.SCR_strWidth( str, font, maxlen );
		}

		static inline size_t SCR_StrlenForWidth( const char *str, struct qfontface_s *font, size_t maxwidth ) {
		...
		static inline char *CL_GetClipboardData( bool primary ) {
		...
		static inline bool Key_IsDown( int keynum ) {
			return UI_IMPORT.Key_IsDown( keynum );
		}

		static inline bool VID_GetModeInfo( int *width, int *height, bool *wideScreen, int mode ) {
			return UI_IMPORT.VID_GetModeInfo( width, height, wideScreen, mode );
		}

		static inline void VID_FlashWindow( int count ) {
		...
		// dynvars
		static inline dynvar_t *Dynvar_Create( const char *name, bool console, dynvar_getter_f getter, dynvar_setter_f setter ) {

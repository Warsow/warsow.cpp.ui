...
namespace UIWsw
{
	class Local
	{
		...
		friend void UI_Init( int vidWidth, int vidHeight, float pixelRatio,
			int protocol, const char *demoExtension, const char *basePath );
		friend void UI_Shutdown( void );
		friend void UI_Refresh( unsigned int time, int clientState, int serverState, 
			bool demoPlaying, const char *demoName, bool demoPaused, unsigned int demoTime, 
			bool backGround, bool showCursor );
		friend void UI_DrawConnectScreen( const char *serverName, const char *rejectmessage, int downloadType, const char *downloadFilename,
		...
		friend void UI_CharEvent( wchar_t key );
	...
	private:
		...
		static struct qfontface_s *fontSystemSmall;
		static struct qfontface_s *fontSystemMedium;
		static struct qfontface_s *fontSystemBig;
	...
	public:
		...
		inline static struct qfontface_s *getFontSmall( void ) { return fontSystemSmall; }
		inline static struct qfontface_s *getFontMedium( void ) { return fontSystemMedium; }
		inline static struct qfontface_s *getFontBig( void ) { return fontSystemBig; }
	};

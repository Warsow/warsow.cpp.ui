...
namespace UICore
{
	...
	{
		if ( button == Importer::mouseWheelUp && scrollbar->isEnabled() )
			scrollbar->doMinus();
		else if ( button == Importer::mouseWheelDown && scrollbar->isEnabled() )
			scrollbar->doPlus();
		else
	...
	void ListBox::updateItemPositions( void )
	{
		...
		if ( horizontal ) {
			for ( it = items.begin() ; it != items.end() ; it++ ) {
				...
				(*it)->setVisible( (pos > -itemSize && pos < getWidth()) ); // track visible items
				pos += (*it)->getWidth();
			}
		} else {
			for ( it = items.begin() ; it != items.end() ; it++) {
				(*it)->setPosition( 0, pos );
				(*it)->setVisible( (pos > -itemSize && pos < getHeight()) ); // track visible items
				pos += (*it)->getHeight();
			}
		}
	}
	...
	bool ListBox::selectItem( ListItem *item )
	{
		if ( item )
		{
			...
			item->setPressed( true );
			selectedItems.insert( std::pair<int, ListItem*>( position, item ) );
	...
	void ListBox::trackScrollbarEnabled( void )
	{
		float itemsVisible;
		if ( horizontal )
			itemsVisible = (getWidth() / itemSize) - 1;
